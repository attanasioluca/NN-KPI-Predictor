...
def convert_to_seconds(input_dict, rng):
        ...
        if type_ == "FIXED":
            ...
            duration = max(0, rng.normal(mean, std_dev))
            ...
            duration = rng.lognormal(mu, math.sqrt(sigma2))
        elif type_ == "GAMMA":
            variance = float(input_dict["arg1"])
            ...
            duration = rng.choice(bin_edges, p=frequencies)
...
class SimulatorEngine:
    ...
    def get_time_until_next_shift(self, timetable_name):
        current_dt = self.start_datetime_obj + timedelta(seconds=self.env.now)
        parsed_rules = self.timetables_dict.get(timetable_name, [])
        
        if not parsed_rules:
            return 60.0  # Fallback if no rules exist
            
        min_wait_seconds = float('inf')
        
        for rule in parsed_rules:
            from_time = rule['from_time']
            from_day_idx = rule['from_day_idx']
            to_day_idx = rule['to_day_idx']
            
            # Check the next 8 days to find the next valid shift start block
            for day_offset in range(8):
                check_dt = current_dt + timedelta(days=day_offset)
                check_day_idx = check_dt.weekday()
                
                # Determine if this weekday falls within the rule's active days
                if from_day_idx <= to_day_idx:
                    is_active_day = from_day_idx <= check_day_idx <= to_day_idx
                else:
                    # Handles week wrap-around (e.g., FRIDAY (4) to MONDAY (0))
                    is_active_day = check_day_idx >= from_day_idx or check_day_idx <= to_day_idx
                    
                if is_active_day:
                    candidate_start = datetime.combine(check_dt.date(), from_time)
                    wait_seconds = (candidate_start - current_dt).total_seconds()
                    
                    # We only care about shift starts strictly in the future
                    if wait_seconds > 0:
                        min_wait_seconds = min(min_wait_seconds, wait_seconds)
                        break # Found the next start for this specific rule
                        
        return min_wait_seconds if min_wait_seconds != float('inf') else 60.0    
    ...
    def update_resources(self,resource_obj, mode, resource_name, node_id): #this function handles setup time for resources
        ...
        setup_time = convert_to_seconds(resource_tuple[4], self.rng)
    ...
    def run_node(self, node_id, subprocess_node=None):
        ...
        if node['type'] == 'startEvent':
            if node['subtype']=="timerEventDefinition":
                ...
                waitTimeSeconds= convert_to_seconds(waitTime, self.rng)
            ...
            taskTime=convert_to_seconds(self.task_durations[node_id], self.rng) # task duration is used here, it is passed before to a converter that transforms the type/mean/arg1/arg2 to a value in seconds, this value the task duration is always different in each instance
            ...
            if taskNeededResources: #if the task needs some resources
                ...
                while True: #iterate till some resources can be allocated
                    ...
                    min_timetable_wait = None  # Replaced the boolean
                    i = 0
                        ...
                        for resource_name, amount_needed in resources:
                            ...
                            if len(available_resources) < amount_needed:
                                ...
                                if len(resources_in_timetable) < amount_needed:
                                    ...
                                    # Calculate exactly how long to sleep for this specific resource
                                    missing_timetables = set(res[2] for res in self.global_resources[resource_name])
                                    wait_times = [self.get_time_until_next_shift(tt) for tt in missing_timetables if tt]
                                    group_wait = min(wait_times) if wait_times else 60.0
                                    
                                    if min_timetable_wait is None or group_wait < min_timetable_wait:
                                        min_timetable_wait = group_wait
                    ...
                    if not resources_allocated:
                        ...
                        if min_timetable_wait is not None:
                            # Apply a 1-second floor to prevent infinite 0-tick looping
                            safe_wait = max(1.0, float(min_timetable_wait))
                            yield self.env.resource_released_event | self.env.timeout(safe_wait)
                        else:
                            yield self.env.resource_released_event
                    else:
        ...
        # add by LR , modify by LR
            ...
            for next_node_id in node['next']:
                ...
                if not nextNode['subtype'] == 'messageEventDefinition': #after event based gateway there is for sure intermediate catch events, if not message i save smallest timer
                    timer=convert_to_seconds(self.catchEvents[next_node_id], self.rng)
